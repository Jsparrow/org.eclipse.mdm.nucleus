 ...
 *
 */
public class SerachFilterParser {
	 ...
	 */
	public static Filter parse(List<EntityType> supported, String filterString) {
		...
		while (filterString.contains(AND_DELIMITER) || filterString.contains(OR_DELIMITER)) {
			...
			int orIndex = filterString.indexOf(OR_DELIMITER) >= 0 ? filterString.indexOf(OR_DELIMITER)
					: Integer.MAX_VALUE;
			if (andIndex < orIndex) {
				filterString = parseAndToken(supported, filterString, filterBuilder);
			} else {
				filterString = parseOrToken(supported, filterString, filterBuilder);
			}
		}
		filterBuilder.addCondition(parseCondition(supported, filterString));
	...
	private static Condition parseCondition(List<EntityType> possibleSearchAttrs,
		...
		if (tokenizer.countTokens() != 3) {
			String message = "unable to parse condition String: " + condition;
	...
	private static Attribute validateAttribute(String entityName, String attrName, 
		List<EntityType> possibleSearchAttrs) {
		...
		String message = "attribute with name '" + entityName + "." + attrName + "' is not supported by this query";
		throw new IllegalArgumentException(message);
	}
	...
	private static Operation stringToOperation(String operationString) {
		...
		case EQUALS_OPERATOR:
			...
			break;
		default:
			String message = "unknown search condition operation: " + operationString;
			throw new IllegalArgumentException(message);
		}
		return operation;
	}
